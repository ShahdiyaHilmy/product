<script lang="ts">
  ...
  function filterAndSortData() {
    let filtered = selectedAlbum ? data.filter(item => item.albumId === parseInt(selectedAlbum, 10)) : data;
    ...
    if (sortOrder === 'asc') {
      ...
      filtered = [...defaultOrder].filter(item => !selectedAlbum || item.albumId === parseInt(selectedAlbum, 10));
      if (searchTerm) {
        filtered = filtered.filter(item => item.title.toLowerCase().includes(searchTerm.toLowerCase()));
      }
    }
...
<div class="max-w-7xl mx-auto bg-white p-4 sm:p-16">
  <main class="flex flex-col gap-10">
    ...
    <div class="flex flex-col sm:flex-row justify-between mb-5 items-center gap-4 sm:gap-8">
      ...
      <select
        class="select px-4 py-2 border rounded w-full sm:w-auto"
      ...
      <button
        class="button px-8 py-2 rounded w-full sm:w-auto"
